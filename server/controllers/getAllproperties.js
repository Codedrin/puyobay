...
export const getAllProperties = async (req, res) => {
  ...
  try {
    ...
    // Filter by available rooms (greater than or equal to provided number of rooms)
    if (rooms) {
      query["rooms.availablePersons"] = { $gte: parseInt(rooms, 10) };
    }
    ...
    // Filter by selected area
    if (selectArea) {
      query.area = selectArea; // Match the exact area
    }
    ...
    // Find properties based on query object and populate room details
