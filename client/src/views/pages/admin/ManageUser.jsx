...
const ManageUser = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get('https://puyobay-server.vercel.app/api/users');
  ...
  const handleToggleApproval = async (landlordId, currentStatus) => {
    try {
      ...
      const response = await axios.put(`https://puyobay-server.vercel.app/api/users/approve/${landlordId}`, { approved: !currentStatus });
  ...
  const handleDeny = async (landlordId) => {
    try {
      await axios.delete(`https://puyobay-server.vercel.app/api/users/deny/${landlordId}`);
