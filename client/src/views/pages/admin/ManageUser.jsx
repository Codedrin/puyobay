...
const ManageUser = () => {
  ...
  const handleToggleApproval = async (landlordId, currentStatus) => {
    try {
      ...
      const response = await axios.put(`https://puyobay.onrender.com/api/users/approve/${landlordId}`, { approved: !currentStatus });
  ...
  const handleDeny = async (landlordId) => {
    try {
      await axios.delete(`https://puyobay.onrender.com/api/users/deny/${landlordId}`);
