...
const LandlordManagedProperty = () => {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        const response = await axios.get(`https://puyobay-server.vercel.app/api/users/landlord-property/${userId}`, {
          params: { userId }
        });
  ...
  const handleDelete = async (propertyId) => {
    if (window.confirm("Are you sure you want to delete this property?")) {
      try {
        await axios.delete(`https://puyobay-server.vercel.app/api/users/delete-property/${propertyId}`);
